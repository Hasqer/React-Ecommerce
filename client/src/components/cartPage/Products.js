...
export default function Products({cartData, userId, setTotalPrice,totalPrice}) {

    const dis = useDispatch();
    const [count, setCount] = useState(1);

    useEffect(() => {
        if(!(totalPrice.some((item) => item.productID == cartData.id))) {
            setTotalPrice(e=> [...e, {
    ...
    useEffect(() => {
        setTotalPrice(e=>
            e.map(element=>{
                if(element.productID == cartData.id) {
                    return{
                        productID:cartData.id,
                        price:cartData.price,
                        count
    ...
    const decrease = (e) =>  {
        if(count > 1){
            ...
            setTotalPrice(e=>e.filter(item=>item.productID != cartData.id))
    ...
    return(
            <div className="container-xl d-flex p-2 m-0 mb-2 py-2 bg-white rounded border d-flex align-items-center justify-content-between">
                ...
                <img src={cartData.photos} className={style.Images + " rounded d-block box-shadow"} alt={cartData.title}/>
                </div>
